    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function register() {
            ...
            console.log(`
                <div>
                    ...
                    <p><a href="${location.origin}/account/verify-email?token=${account.verificationToken}">${location.origin}/account/verify-email?token=${account.verificationToken}</a></p>
                </div>
            `);
        ...
        function error(message: string) {
            return throwError(() => ({ error: { message } }))
                .pipe(materialize(), delay(500), dematerialize()); // call materialize and dematerialize to ensure delay
        }
